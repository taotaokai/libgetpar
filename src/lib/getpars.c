...
static void tabulate (size_t argc, char **argv)
{
  ...
  for (i = 0, nargs = 0 ; (size_t)i < argc; i++) {
    eqptr = strchr(argv[i], '=');
    if (eqptr) {
...
/* copy the content of argtbl
 *
 * name[]: array of pointers to the parameter names in argv[]
 * asciival[]: array of pointer to the parameter value in argv[]
 * size: the size of name[] and asciival[]
 *
 * only copy min(nargs,size) elements to name[] and asciival[];
 *
 * Return: nargs;
 */
int getargtbl(char *name[], char *asciival[], int size)
{
  ...
  for (i=0; (i<nargs)&&(i<size); i++) {
    name[i] = argtbl[i].name;
    asciival[i] = argtbl[i].asciival;
  }
